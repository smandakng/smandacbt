...
async function setDoc(ref, payload, options = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const missing = extractMissingSelectColumn(error);
    if (missing && dataToSave[missing] !== undefined) {
      console.warn(`setDoc ${ref.collectionName}: kolom "${missing}" belum ada di remote DB, retry tanpa kolom itu`);
      delete dataToSave[missing];
      continue;
    }

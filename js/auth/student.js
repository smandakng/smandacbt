...
function renderAppSettingsUI() {
  ...
  const currentFooter = String(settings.footerText || '').trim();
  const validFooter = (!currentFooter || currentFooter !== STANDARD_FOOTER_TEXT) ? STANDARD_FOOTER_TEXT : currentFooter;
  if (footerText) footerText.textContent = validFooter;

...
function updateAdminTokenBars(force = false) {
  ...
  const getScheduleCreationTime = (s) => {
    if (!s) return -1;
    if (typeof s.id === 'string') {
      const match = s.id.match(/^sch_(\d+)$/);
      if (match) return Number(match[1]);
    }
    const startMs = s.mulai ? new Date(s.mulai).getTime() : 0;
    return Number.isFinite(startMs) ? startMs : 0;
  };

  const g = (regex) => {
    const matching = ALL_SCHEDULES.filter(s => Array.isArray(s.kelas_terpilih) && s.kelas_terpilih.some(c => regex.test(c)));
    if (!matching.length) return null;
    return matching.reduce((latest, current) => {
      if (!latest) return current;
      return getScheduleCreationTime(current) > getScheduleCreationTime(latest) ? current : latest;
    }, null);
  };

function renderDashboardActiveExamsTable() {
  ...
  if (!ALL_SCHEDULES.length) {
    tbody.innerHTML = `<tr><td colspan="7" class="py-8 text-center text-slate-400 font-bold text-xs">Belum ada jadwal ujian yang terdaftar</td></tr>`;
    return;
  }
  ...
  if (!filteredSchedules.length) {
    tbody.innerHTML = `<tr><td colspan="7" class="py-8 text-center text-slate-400 font-bold text-xs">Jadwal ujian tidak ditemukan</td></tr>`;
  ...
  displaySchedules.forEach(sch => {
    ...
    tr.innerHTML = `
      ...
      <td class="py-3 px-4 font-mono font-bold text-amber-600 dark:text-amber-400 text-xs sm:text-sm tracking-wider">${esc(sch.token || '-')}</td>
      <td class="py-3 px-4">

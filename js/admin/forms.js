...
async function handleAddSchedule() {
  ...
  if (!ALL_PACKETS.length) {
    showNotification("Kosong", "Buat soal dulu", "danger");
    return;
  }
    ...
    <div class="space-y-3 text-base text-left text-slate-800 dark:text-slate-100">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <input type="datetime-local" id="s-dt" oninput="window.syncScheduleTime('s', 'start')" onchange="window.syncScheduleTime('s', 'start')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Durasi (Menit)</label>
          <input type="number" id="s-dr" value="60" min="1" placeholder="60" oninput="window.syncScheduleTime('s', 'duration')" onchange="window.syncScheduleTime('s', 'duration')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Waktu Selesai</label>
          <input type="datetime-local" id="s-dt-end" oninput="window.syncScheduleTime('s', 'end')" onchange="window.syncScheduleTime('s', 'end')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950">
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('X', this.checked, 'sc')" class="rounded text-primary cursor-pointer">
        ...
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('XI', this.checked, 'sc')" class="rounded text-primary cursor-pointer">
        ...
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('XII', this.checked, 'sc')" class="rounded text-primary cursor-pointer">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950">
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-as" checked class="rounded text-primary cursor-pointer">
          <span class="text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">Acak Soal</span>
        </label>
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-aj" checked class="rounded text-primary cursor-pointer">
          <span class="text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">Acak Jawaban</span>
        </label>
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-tn" class="rounded text-primary cursor-pointer">
  ...
  `, async () => {
    ...
    const drInput = parseInt(document.getElementById('s-dr').value, 10);
    const dr = (!isNaN(drInput) && drInput > 0)
      ? drInput
      : ((dt && dtEnd) ? Math.max(1, Math.round((new Date(dtEnd) - new Date(dt)) / 60000)) : 60);
    const cl = Array.from(document.querySelectorAll('input[name="sc"]:checked')).map(cb => cb.value);
    const as = document.getElementById('s-as').checked;
    const aj = document.getElementById('s-aj').checked;
    ...
    try {
      ...
      const newSched = { id, mapel: m, id_paket: p, mulai: times.mulai, selesai: times.selesai, durasi: times.durasi, kelas_terpilih: cl, token: tk, acak_soal: as, acak_jawaban: aj, tampil_nilai: tn };

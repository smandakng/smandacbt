...
window.syncScheduleTime = function (prefix, triggerSource) {
  const startEl = document.getElementById(prefix === 's-edit' ? 's-dt-edit' : 's-dt');
  const durEl = document.getElementById(prefix === 's-edit' ? 's-dr-edit' : 's-dr');
  const endEl = document.getElementById(prefix === 's-edit' ? 's-dt-end-edit' : 's-dt-end');
  ...
  if (triggerSource === 'start' || triggerSource === 'duration') {
    const durMinutes = parseInt(durEl.value, 10);
    if (!isNaN(durMinutes) && durMinutes > 0) {
      const endDate = new Date(startDate.getTime() + durMinutes * 60000);
      const year = endDate.getFullYear();
      const month = String(endDate.getMonth() + 1).padStart(2, '0');
      const day = String(endDate.getDate()).padStart(2, '0');
      const hours = String(endDate.getHours()).padStart(2, '0');
      const minutes = String(endDate.getMinutes()).padStart(2, '0');
      endEl.value = `${year}-${month}-${day}T${hours}:${minutes}`;
    }
    ...
    if (!isNaN(endDate.getTime()) && endDate > startDate) {
      ...
      durEl.value = Math.max(1, diffMinutes);
    }
  }
};

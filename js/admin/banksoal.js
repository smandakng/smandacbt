...
function resetManualQuestionFormState() {
  ...
  window.currentEditingPacketId = null;
...
function populateManualQuestionForm(packetId, question) {
  ...
  ['A', 'B', 'C', 'D', 'E'].forEach(letter => {
    ...
    if (optionInput && optionImageAsset && !/<img[\s\S]*src=['\"]?data:image/i.test(optionInput.innerHTML || '')) {
      optionInput.innerHTML += `<div class="manual-option-img-wrapper"><img src="${optionImageAsset.data}" alt="Gambar" style="max-width:100%; height:auto; display:block; margin-top:0.5rem; border-radius:1rem;"></div>`;
    }
  });

  window.currentEditingQuestionId = question.id;
  window.currentEditingPacketId = packetId;
  const saveBtn = document.getElementById('btn-save-manual-question');
...
async function handleSaveManualQuestion() {
  ...
  const origPacketId = window.currentEditingPacketId;
  ...
  for (const letter of opsiKeys) {
    ...
    let textWithoutImages = textRaw
      .replace(/<div[^>]*class=['"]manual-option-img-wrapper['"][^>]*>[\s\S]*?<\/div>/gi, '')
      .replace(/<img[^>]*>/gi, '')
      .trim();
    const text = textWithoutImages ? sanitizeHtmlContent(stripQuestionKeyFromHtml(textWithoutImages)).trim() : '';
  ...
  try {
    // If editing and packet was changed from original packet to another packet
    if (isEditing && origPacketId && origPacketId !== targetPacketId) {
      const origPacket = await ensureAdminPacketLoaded(origPacketId);
      if (origPacket) {
        const origQuestions = (origPacket.daftar_soal || [])
          .filter(q => String(q.id) !== String(editingQuestionId))
          .map((q, idx) => ({ ...q, nomer: idx + 1 }));
        const updatedOrigPacket = typeof normalizeBankSoalPacket === 'function'
          ? normalizeBankSoalPacket({ ...origPacket, daftar_soal: origQuestions, konten_versi: new Date().toISOString() })
          : { ...origPacket, daftar_soal: origQuestions, konten_versi: new Date().toISOString() };
        await publishBankSoalPacket(updatedOrigPacket);
        await setDoc(getPublicDoc('Bank Soal', origPacketId), updatedOrigPacket);
        if (typeof setCachedPacket === 'function') setCachedPacket(origPacketId, updatedOrigPacket);
        const origIdx = ALL_PACKETS.findIndex(p => p.id_paket === origPacketId);
        if (origIdx >= 0) ALL_PACKETS[origIdx] = updatedOrigPacket;
      }
    }

    const updatedPacket = { ...packet };
    const existsInTarget = (packet.daftar_soal || []).some(q => String(q.id) === String(editingQuestionId));

    if (isEditing && !isNewPacket && existsInTarget) {
      updatedPacket.daftar_soal = (packet.daftar_soal || []).map(q => String(q.id) === String(editingQuestionId) ? {
        ...q,
    ...
    if (typeof setCachedPacket === 'function') {
      setCachedPacket(targetPacketId, record);
    }
...
window.triggerEditBankSoalQuestion = function (packetId, questionId) {
  ...
  const question = packet?.daftar_soal?.find(q => String(q.id) === String(questionId));
...
window.triggerDeleteBankSoalQuestion = function (packetId, questionId) {
  ...
  showConfirmation('Hapus Soal', 'Yakin ingin menghapus soal ini?', async () => {
    ...
    try {
      ...
      const updatedQuestions = (packet.daftar_soal || []).filter(q => String(q.id) !== String(questionId)).map((q, idx) => ({ ...q, nomer: idx + 1 }));

...
window.triggerDeleteSchedule = function (id) {
  ...
  showConfirmation("Hapus", "Hapus jadwal?", async () => {
    ...
    try {
      ...
      if (typeof updateAdminTokenBars === 'function') updateAdminTokenBars(true);
      renderSchedules();
...
window.triggerEditSchedule = function (id) {
  ...
  const asChecked = sch.acak_soal !== false ? 'checked' : '';
  const ajChecked = sch.acak_jawaban !== false ? 'checked' : '';
  ...
  const durasiVal = (sch.durasi && Number(sch.durasi) > 0)
    ? Number(sch.durasi)
    : ((sch.mulai && sch.selesai) ? Math.max(1, Math.round((new Date(sch.selesai) - new Date(sch.mulai)) / 60000)) : 60);
    ...
    <div class="space-y-3 text-base text-left text-slate-800 dark:text-slate-100">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <input type="datetime-local" id="s-dt-edit" value="${esc(mulaiVal)}" oninput="window.syncScheduleTime('s-edit', 'start')" onchange="window.syncScheduleTime('s-edit', 'start')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Durasi (Menit)</label>
          <input type="number" id="s-dr-edit" value="${durasiVal}" min="1" placeholder="60" oninput="window.syncScheduleTime('s-edit', 'duration')" onchange="window.syncScheduleTime('s-edit', 'duration')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Waktu Selesai</label>
          <input type="datetime-local" id="s-dt-end-edit" value="${esc(selesaiVal)}" oninput="window.syncScheduleTime('s-edit', 'end')" onchange="window.syncScheduleTime('s-edit', 'end')" class="w-full p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900 outline-none focus:ring-2 focus:ring-primary/50 text-base">
        </div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950">
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('X', this.checked, 'sc-edit')" class="rounded text-primary cursor-pointer">
        ...
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('XI', this.checked, 'sc-edit')" class="rounded text-primary cursor-pointer">
        ...
        <div class="space-y-3 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-visible">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" onchange="window.toggleGlobalClass('XII', this.checked, 'sc-edit')" class="rounded text-primary cursor-pointer">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-2.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950">
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-as-edit" ${asChecked} class="rounded text-primary cursor-pointer">
          <span class="text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">Acak Soal</span>
        </label>
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-aj-edit" ${ajChecked} class="rounded text-primary cursor-pointer">
          <span class="text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">Acak Jawaban</span>
        </label>
        <label class="flex items-center gap-2.5 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer select-none">
          <input type="checkbox" id="s-tn-edit" ${tnChecked} class="rounded text-primary cursor-pointer">
          <span class="text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">Tampilkan Nilai</span>
        </label>
      </div>
    </div>
  `, async () => {
    ...
    const drInput = parseInt(document.getElementById('s-dr-edit').value, 10);
    const dr = (!isNaN(drInput) && drInput > 0)
      ? drInput
      : ((dt && dtEnd) ? Math.max(1, Math.round((new Date(dtEnd) - new Date(dt)) / 60000)) : (sch.durasi || 60));
    const cl = Array.from(document.querySelectorAll('input[name="sc-edit"]:checked')).map(cb => cb.value);
    const as = document.getElementById('s-as-edit').checked;
    const aj = document.getElementById('s-aj-edit').checked;
    ...
    try {
      ...
      try {
        if (typeof supabaseClient !== 'undefined' && supabaseClient) {
          supabaseClient.channel(`cbt-schedule-broadcast-${id}`).send({
            type: 'broadcast',
            event: 'schedule_updated',
            payload: { schedule: updatedSch }
          }).catch(() => {});
        }
      } catch (_) {}
...
window.triggerResetIndividu = function (sid) {
  ...
  showConfirmation("Reset Ujian", "Yakin ingin mereset sesi ujian siswa ini?", async () => {
    try {
      ...
      console.error('triggerResetIndividu failed', e);

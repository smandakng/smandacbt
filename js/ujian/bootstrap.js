function showExamRulesDialog() {
  const maxAllowed = typeof getMaxExamViolations === 'function' ? getMaxExamViolations() : 3;
  showConfirmation("", `<div class="dialog-warning-content text-left space-y-3 leading-relaxed">
      ...
      <p>Jika Anda melakukan pelanggaran hingga ${maxAllowed} kali, ujian akan dihentikan secara otomatis dan nilai hanya dihitung dari soal yang sudah dijawab. "Pastikan koneksi internet stabil sebelum memulai."</p>
      <p class="dialog-warning-heading uppercase tracking-wide">PERINGATAN :</p>
      <ul class="list-disc pl-5 space-y-0">
        <li>Wajib Scan Barcode menggunakan aplikasi CBT/SEB</li>
        <li>Wajib Sematkan Aplikasi agar jawaban tersimpan</li>
        <li>Pastikan layar HP tidak tertutup otomatis / tidak aktif</li>
        <li>Wajib <b>di Reset</b> oleh Admin jika keluar dari halaman ujian, beritahu pengawas NIS dan Nama Siswa</li>
      </ul>
...
async function cancelExamSession() {
  ...
  if (typeof stopStudentScheduleRealtimeUpdates === 'function') stopStudentScheduleRealtimeUpdates();
...
async function initStudentExamView() {
  toggleLoader(true, "Mempersiapkan ujian...");
  try {
    ...
    if (!sched) {
      const schedCols = 'id,mapel,id_paket,mulai,selesai,durasi,kelas_terpilih,token,tampil_nilai,acak_soal,acak_jawaban';
    ...
    if (savedProgress) {
      ...
      EXAM_STATE.cheatTabCount = Number.isInteger(merged.cheatTabCount) ? merged.cheatTabCount : (EXAM_STATE.cheatTabCount || 0);
      let frozenSeconds = typeof merged.frozenTimeRemaining === 'number'
        ? merged.frozenTimeRemaining
        : merged.timeRemaining;

      const savedDur = Number(merged.scheduleDurasi) || 0;
      const currentDur = Number(sched.durasi) || 0;
      const savedEndMs = Number(merged.scheduleEndMs) || 0;
      const currentEndMs = typeof getExamScheduleEndMs === 'function' ? getExamScheduleEndMs(sched) : 0;

      let durDiffSec = 0;
      if (savedEndMs > 0 && currentEndMs > 0 && currentEndMs !== savedEndMs) {
        durDiffSec = Math.round((currentEndMs - savedEndMs) / 1000);
      } else if (savedDur > 0 && currentDur > 0 && currentDur !== savedDur) {
        durDiffSec = (currentDur - savedDur) * 60;
      }

      if (durDiffSec !== 0 && typeof frozenSeconds === 'number') {
        frozenSeconds = Math.max(0, frozenSeconds + durDiffSec);
      }
      ...
      const originalQuestions = pkt.daftar_soal || [];
      EXAM_STATE.scrambledQuestions = sched.acak_soal !== false
        ? shuffleArray(originalQuestions)
        : [...originalQuestions];

      EXAM_STATE.scrambledQuestions.forEach(q => {
        const originalOpts = q.opsi || [];
        const opts = sched.acak_jawaban !== false
          ? shuffleArray(originalOpts)
          : [...originalOpts].sort((a, b) => String(a.key || '').localeCompare(String(b.key || '')));

        EXAM_STATE.scrambledOptions[q.id] = opts.map((o, idx) => ({
          ...o,

function buildExamProgressPayload(examState) {
  ...
  return {
    ...
    timerFrozen: true,
    scheduleDurasi: examState.schedule?.durasi || null,
    scheduleEndMs: typeof getExamScheduleEndMs === 'function' ? getExamScheduleEndMs(examState.schedule) : null,
...
function extractExamProgressFields(state) {
  ...
  const result = {
    ...
    scheduleDurasi: state.scheduleDurasi || null,
    scheduleEndMs: state.scheduleEndMs || null,
...
function mergeSavedExamWithBankSoal(savedState, packetData) {
  ...
  scrambledQuestions.forEach((q) => {
    ...
    if (!scrambledOptions[q.id]) {
      ...
      ).map((o, idx) => ({ ...o, displayKey: String.fromCharCode(65 + idx) }));
      return;
    }
    ...
    scrambledOptions[q.id] = (rebuilt.length ? rebuilt : [...bankOpsi]).map((o, idx) => ({
      ...o,
